...
func (m model) codeburnView(width int) string {
	title := segB(orange, " Agent Usage") + seg(muted, "  · via CodeBurn · "+displayRoot(m.root))
...
func (m model) codeburnSummary(width int) string {
	...
	c := p.Current
	marker := segB(orange, "▌ ")
	content := marker + segB(orange, "AGENT USAGE") + seg(muted, "  via CodeBurn · "+c.Label+"   ") +
...
// showLegacyUsageSummary keeps Orchard's all-history transcript aggregate as a
// fallback. It is intentionally hidden when CodeBurn data is available because
// the two summaries use different time windows and token normalization.
func (m model) showLegacyUsageSummary() bool {
	return m.codeburnPayload == nil && m.showClaudePanel()
}

func (m model) legacyUsageSummaryRows() int {
	if m.codeburnPayload != nil {
		return 0
	}
	return m.claudePanelRows()
}
...
func burnOverview(payload *codeburn.Payload, width int) []string {
	...
	return burnBox("Agent Usage  "+c.Label, orange, body, width, len(body))
}
...
func burnPanels(payload *codeburn.Payload) []burnPanel {
	...
	return []burnPanel{
		...
		{title: "By Model", color: accent, body: func(width int) []string {
			...
			rows := burnRanked(limitBurnEntries(entries, 8), width, "No model usage")
			if note := burnUnpricedNote(c.UnpricedModels, width); note != "" {
...
func burnUnpricedNote(models []codeburn.UnpricedModel, width int) string {
	if len(models) == 0 {
		return ""
	}
	...
	if detail == "" {
		detail = fmt.Sprintf("%d model%s", len(models), pluralSuffix(len(models)))
	}
	if calls > 0 {
		detail += fmt.Sprintf(" · %d calls", calls)
	}
	return seg(yellow, fit("! unpriced: "+detail, width))
}
